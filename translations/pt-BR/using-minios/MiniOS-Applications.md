...
| Tarefa | Ferramenta | Escopo | Documentação |
|---|---|---|---|
| Listar discos de destino, visualizar um plano de implantação ou instalar MiniOS de forma não interativa | **`minios-deploy`** | Executa a partir de uma sessão live MiniOS. A instalação requer root e confirmação explícita; o modo nativo, quando suportado, cria um desktop Debian convencional a partir da imagem selecionada. | [Instalador do MiniOS](/installing-minios/MiniOS-Installer#command-line-deployment); `man minios-deploy` |
| Criar, ativar, salvar, redimensionar, exportar, importar ou remover sessões persistentes | **`minios-session`** | Requer root e um sistema live MiniOS compatível com armazenamento de persistência. | [Gerenciamento de sessões](/using-minios/Sessions-and-Persistence#command-reference); `man minios-session` |

### Kernels e imagens

| Tarefa | Ferramenta | Escopo | Documentação |
|---|---|---|---|
| Listar, empacotar, ativar, inspecionar ou remover kernels | **`minios-kernel`** | Requer root e uma instalação modular do MiniOS live com root MiniOS gravável. | [Gerenciamento de kernels](/preparing-and-customizing/Managing-Kernels#method-2-using-minios-kernel-cli); `man minios-kernel` |
| Remasterizar uma árvore de conteúdo MiniOS existente a partir de scripts ou automação | **`minios-image-compose`** | Opera sobre o conteúdo da imagem live MiniOS e publica um ISO inicializável. | [Composição de imagens ISO pela linha de comando](/preparing-and-customizing/Creating-Custom-MiniOS-Images); `man minios-image-compose` |

### Fluxos de trabalho com módulos

| Tarefa | Ferramenta | Escopo | Documentação |
|---|---|---|---|
| Inspecionar módulos e gerenciar os conjuntos de módulos ativos ou do próximo boot | **`sb`** | A inspeção de módulos também funciona fora de uma sessão MiniOS em execução. Operações de execução e próximo boot requerem um layout de módulos live MiniOS; alterações exigem root. | [Criação de módulos](/preparing-and-customizing/Managing-Modules); `man sb` |
| Construir um módulo a partir de pacotes do repositório ou arquivos locais `.deb` | **`apt2sb`** | Requer root e uma sessão live MiniOS suportada. Os pacotes são instalados em um ambiente de build isolado, não no root em execução. | [Criação de módulos](/preparing-and-customizing/Managing-Modules#create-a-module-from-packages); `man apt2sb` |
| Construir um módulo executando um script de instalação | **`script2sb`** | Requer root e uma sessão live MiniOS suportada. O script é executado de forma não interativa em um ambiente de build isolado. | [Criação de módulos](/preparing-and-customizing/Managing-Modules#create-a-module-from-a-script); `man script2sb` |
| Construir um módulo interativamente em um ambiente preparado | **`chroot2sb`** | Requer root e uma sessão live MiniOS suportada. Use quando a instalação exigir prompts ou alterações manuais. | [Criação de módulos](/preparing-and-customizing/Managing-Modules#create-a-module-interactively); `man chroot2sb` |
| Converter entre uma árvore de diretórios e um módulo `.sb` | **`dir2sb`**, **`sb2dir`** | A conversão comum não requer root e pode ser usada fora de uma sessão live em execução, desde que as ferramentas e arquivos necessários estejam disponíveis. | [Criar](/preparing-and-customizing/Managing-Modules#create-a-module-from-a-directory) ou [extrair](/preparing-and-customizing/Managing-Modules#inspect-and-extract-modules) um módulo; `man dir2sb`, `man sb2dir` |
| Capturar alterações elegíveis da camada gravável da sessão em um módulo | **`savechanges`** | Requer root e uma sessão live MiniOS em execução com backend de camada gravável suportado. | [Criação de módulos](/preparing-and-customizing/Managing-Modules#capture-current-session-changes); `man savechanges` |
