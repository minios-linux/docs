...
Etcher realiza una escritura de imagen en bruto: copia la estructura del ISO en todo el dispositivo de destino. No crea una partición ext4 en el espacio no utilizado, no crea una sesión de persistencia ni realiza un despliegue del Instalador de MiniOS.

La persistencia solo se activa cuando una entrada de arranque o una línea de comandos del kernel la solicita, y aún requiere un almacenamiento adecuado con permisos de escritura. Consulta [Modos de arranque](/using-minios/Boot-Modes) y [Persistencia de Initrd](/reference/boot-process/Persistence-Internals) antes de depender de los cambios guardados.
