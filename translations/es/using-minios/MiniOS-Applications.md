...
| Tarea | Herramienta | Alcance | Documentación |
|---|---|---|---|
| Listar discos de destino, previsualizar un plan de despliegue o instalar MiniOS de forma no interactiva | **`minios-deploy`** | Se ejecuta desde una sesión en vivo de MiniOS. La instalación requiere permisos de root y confirmación explícita; el modo nativo, cuando está disponible, crea un escritorio Debian convencional a partir de la imagen seleccionada. | [Instalador de MiniOS](/installing-minios/MiniOS-Installer#command-line-deployment); `man minios-deploy` |
| Crear, activar, guardar, redimensionar, exportar, importar o eliminar sesiones persistentes | **`minios-session`** | Requiere permisos de root y un sistema en vivo de MiniOS con un almacén de persistencia compatible. | [Gestión de sesiones](/using-minios/Sessions-and-Persistence#command-reference); `man minios-session` |

### Kernels e imágenes

| Tarea | Herramienta | Alcance | Documentación |
|---|---|---|---|
| Listar, empaquetar, activar, inspeccionar o eliminar kernels | **`minios-kernel`** | Requiere permisos de root y una instalación modular en vivo de MiniOS con una raíz MiniOS escribible. | [Gestión de kernels](/preparing-and-customizing/Managing-Kernels#method-2-using-minios-kernel-cli); `man minios-kernel` |
| Remasterizar un árbol de contenido existente de MiniOS desde scripts o automatización | **`minios-image-compose`** | Opera sobre el contenido de la imagen en vivo de MiniOS y publica una ISO arrancable. | [Composición de imágenes ISO desde la línea de comandos](/preparing-and-customizing/Creating-Custom-MiniOS-Images); `man minios-image-compose` |

### Flujos de trabajo de módulos

| Tarea | Herramienta | Alcance | Documentación |
|---|---|---|---|
| Inspeccionar módulos y gestionar los conjuntos de módulos en ejecución o para el próximo arranque | **`sb`** | La inspección de módulos también funciona fuera de una sesión en vivo de MiniOS. Las operaciones en ejecución y para el próximo arranque requieren un diseño de módulos en vivo de MiniOS; los cambios requieren permisos de root. | [Creación de módulos](/preparing-and-customizing/Managing-Modules); `man sb` |
| Construir un módulo a partir de paquetes de repositorio o archivos locales `.deb` | **`apt2sb`** | Requiere permisos de root y una sesión en vivo compatible de MiniOS. Los paquetes se instalan en un entorno de compilación aislado, no en la raíz en ejecución. | [Creación de módulos](/preparing-and-customizing/Managing-Modules#create-a-module-from-packages); `man apt2sb` |
| Construir un módulo ejecutando un script de instalación | **`script2sb`** | Requiere permisos de root y una sesión en vivo compatible de MiniOS. El script se ejecuta de forma no interactiva en un entorno de compilación aislado. | [Creación de módulos](/preparing-and-customizing/Managing-Modules#create-a-module-from-a-script); `man script2sb` |
| Construir un módulo de manera interactiva en un entorno preparado | **`chroot2sb`** | Requiere permisos de root y una sesión en vivo compatible de MiniOS. Úsalo cuando la instalación requiera indicaciones o cambios manuales. | [Creación de módulos](/preparing-and-customizing/Managing-Modules#create-a-module-interactively); `man chroot2sb` |
| Convertir entre un árbol de directorios y un módulo `.sb` | **`dir2sb`**, **`sb2dir`** | La conversión ordinaria no requiere root y puede usarse fuera de una sesión en vivo cuando las herramientas y archivos de entrada necesarios están disponibles. | [Crear](/preparing-and-customizing/Managing-Modules#create-a-module-from-a-directory) o [extraer](/preparing-and-customizing/Managing-Modules#inspect-and-extract-modules) un módulo; `man dir2sb`, `man sb2dir` |
| Capturar los cambios elegibles de la capa de sesión escribible en un módulo | **`savechanges`** | Requiere permisos de root y una sesión en vivo de MiniOS en ejecución con un backend de capa escribible compatible. | [Creación de módulos](/preparing-and-customizing/Managing-Modules#capture-current-session-changes); `man savechanges` |
