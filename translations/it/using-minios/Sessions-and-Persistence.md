...
Qualsiasi parametro di persistenza riconosciuto abilita la gestione della persistenza. I menu di avvio MiniOS normalmente offrono voci per ripristino, nuova sessione, selezione e avvio non persistente. La descrizione canonica di selettore, compatibilità, fallback e semantica di attivazione è disponibile in [Persistenza nell’initrd](/reference/boot-process/Persistence-Internals).

| Parametro | Significato |
|-----------|---------|
| `perch` | Usa il percorso legacy di ripristino best-effort. Tenta di utilizzare il valore predefinito dei metadati ma non crea una sostituzione se nessuno è utilizzabile. |
| `perchdir=resume` | Ripristina il valore predefinito dei metadati e, se assente o incompatibile, consente all’initrd di creare una nuova sostituzione compatibile. Questo è il comportamento attuale del menu di avvio per il ripristino. |
...
| `perchsize=<size>` | Imposta una dimensione del contenitore nuova o maggiore; i valori semplici sono allocati in MiB e sono accettati i suffissi `MB`, `GB` e `TB`. |

Se non viene specificata una modalità per una nuova sessione, l’avvio utilizza la modalità nativa. Su FAT32/NTFS/exFAT, la creazione nativa in fase di avvio ricade su DynFileFS. Un nuovo contenitore raw o LUKS di avvio ha come valore predefinito 4000 MiB; una nuova sessione DynFileFS di avvio senza `perchsize` viene dimensionata in base allo spazio disponibile mantenendo una riserva di sicurezza.
...
Durante il ripristino, MiniOS verifica la versione, l’edizione, il filesystem union e la modalità registrati. Il comando letterale `perchdir=resume` può creare una nuova sessione invece di usare un valore predefinito assente o incompatibile. `perch` senza parametri, selezione numerica diretta e altre richieste legacy di ripristino non creano automaticamente tale sostituzione.
...
Si tratta di metadati persistenti scritti dall’implementazione di avvio, ma non costituiscono di per sé prova dello stato di runtime attuale. Non modificarli né spostare i dati delle sessioni numerate mentre una sessione è montata; utilizza il Gestore sessioni MiniOS o `minios-session`.
...
- Concettualmente, la sessione **in esecuzione** è quella il cui layer scrivibile fornisce effettivamente la persistenza all'avvio corrente.

Il campo persistente `running=` registra questa relazione prevista. Un crash, la costruzione fallita dell'union, la copia dello store o uno spegnimento interrotto possono lasciarlo obsoleto anche se l'avvio corrente sta usando RAM o un'altra sessione. Operazioni come il salvataggio SquashFS richiedono quindi lo stato protetto dell'initrd, vincolato all'ID di avvio corrente e l'upper montato verificato; non si affidano solo a `running=`. Vedi [Stato attivo, in esecuzione e di avvio corrente](/reference/boot-process/Persistence-Internals).
...
`create` senza modalità seleziona quella nativa. La creazione SquashFS cattura le modifiche live correnti e non ha una dimensione fissa. La sua policy di salvataggio allo spegnimento è impostata di default su `shutdown`; il salvataggio periodico è disattivato per impostazione predefinita.
...
Gli intervalli periodici validi sono `30`, `60`, `120`, `240` e `480` minuti; `0` disabilita il salvataggio periodico. Le impostazioni di spegnimento e periodiche sono indipendenti.
...
`copy` assegna sempre un nuovo ID sessione. `convert` sostituisce la sorgente per impostazione predefinita; usa `--new-session` per preservare la sorgente. Una dimensione è rilevante solo per un target contenitore.
...
Il ridimensionamento supporta le sessioni DynFileFS, raw e LUKS e richiede una dimensione superiore a quella attuale. La pulizia di default riguarda le sessioni più vecchie di 30 giorni.

Tutti i comandi accettano `--json` e può essere selezionato uno store di sessioni diverso con `--sessions-dir PATH`:
...
Non viene mantenuta alcuna generazione di rollback. "Salva ora" è disponibile dall’icona nella tray, dal Gestore sessioni MiniOS o da `minios-session save` indipendentemente dalla policy automatica.

Il salvataggio allo spegnimento è gestito dal trigger di spegnimento core MiniOS e dal backend `minios-squashfs-save`, quindi non dipende dal fatto che il Gestore sessioni MiniOS sia aperto o installato. Il salvataggio periodico viene controllato ogni 30 minuti da un timer systemd o da un worker SysV, entrambi richiamano lo stesso backend di autosalvataggio. La ricostruzione dello snapshot utilizza CPU e scrive l’intero snapshot; si consiglia un intervallo di almeno un’ora.

Durante il funzionamento RAM-backed SquashFS, uno snapshot SquashFS appena acquisito e attivato può prendere possesso del target di salvataggio attivo. Dopo questo passaggio, il vecchio snapshot attivo può essere rimosso senza riavviare:
...
Questa eccezione si applica solo a un passaggio valido dello snapshot SquashFS dell’avvio corrente. Le altre modalità di persistenza attive restano protette dalla cancellazione.
...
Per le sessioni native, DynFileFS, raw e LUKS, utilizza `export` per i backup invece di copiare una directory di sessione montata. Conserva l’archivio risultante su un altro dispositivo e verifica che possa essere importato prima di farvi affidamento. L’importazione crea sempre una nuova sessione numerata; attivala esplicitamente quando è pronta all’uso.
Per le procedure di backup SquashFS e di intero dispositivo, consulta [Backup di MiniOS](/maintenance-and-recovery/Backing-Up-MiniOS).

Se una sessione fallisce dopo che lo spazio di archiviazione si è esaurito, una scrittura viene interrotta o vengono create ripetutamente sessioni vuote, interrompi ogni modifica allo storage interessato. Esporta prima una sessione non attiva e leggibile quando possibile, quindi segui la sezione [Risoluzione dei problemi](/maintenance-and-recovery/Troubleshooting).
...
All’avvio, i filesystem dei contenitori vengono controllati prima dell’attivazione in scrittura. Errori gravi nel controllo del filesystem preservano il contenitore per il recupero invece di montarlo in scrittura. SquashFS rileva uno stato precedente non pulito e ripristina l’ultimo snapshot salvato con successo. Elimina le sessioni solo tramite il Gestore sessioni MiniOS o `minios-session delete`; non rimuovere manualmente le directory delle sessioni.
