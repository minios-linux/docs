...
| Tâche | Outil | Portée | Documentation |
|---|---|---|---|
| Lister les disques cibles, prévisualiser un plan de déploiement ou installer MiniOS sans interaction | **`minios-deploy`** | Fonctionne depuis une session live MiniOS. L'installation nécessite les droits root et une confirmation explicite ; le mode natif, si disponible, crée un bureau Debian classique à partir de l'image sélectionnée. | [Déploiement en ligne de commande](/installing-minios/MiniOS-Installer#command-line-deployment); `man minios-deploy` |
| Créer, activer, sauvegarder, redimensionner, exporter, importer ou supprimer des sessions persistantes | **`minios-session`** | Nécessite les droits root et un système live MiniOS avec un espace de persistance compatible. | [Référence des commandes de session](/using-minios/Sessions-and-Persistence#command-reference); `man minios-session` |

### Noyaux et images

| Tâche | Outil | Portée | Documentation |
|---|---|---|---|
| Lister, empaqueter, activer, inspecter ou supprimer des noyaux | **`minios-kernel`** | Nécessite les droits root et une installation live modulaire MiniOS avec un root MiniOS accessible en écriture. | [`minios-kernel` CLI](/preparing-and-customizing/Managing-Kernels#method-2-using-minios-kernel-cli); `man minios-kernel` |
| Recomposer un arbre de contenu MiniOS existant à partir de scripts ou d'automatisation | **`minios-image-compose`** | Agit sur le contenu d'image live MiniOS et publie une ISO amorçable. | [Composer des images ISO en ligne de commande](/preparing-and-customizing/Creating-Custom-MiniOS-Images#composing-minios-iso-images-from-the-command-line); `man minios-image-compose` |

### Flux de travail des modules

| Tâche | Outil | Portée | Documentation |
|---|---|---|---|
| Inspecter les modules et gérer les ensembles de modules actifs ou au prochain démarrage | **`sb`** | L’inspection des modules fonctionne aussi hors d’une session MiniOS en cours. Les opérations sur les modules actifs ou au prochain démarrage nécessitent une disposition de modules live MiniOS ; les modifications requièrent les droits root. | [Inspecter et extraire des modules](/preparing-and-customizing/Managing-Modules#inspect-and-extract-modules); [gérer les modules actifs et au prochain démarrage](/preparing-and-customizing/Managing-Modules#manage-running-and-next-boot-modules); `man sb` |
| Construire un module à partir de paquets du dépôt ou de fichiers locaux`.deb` fichiers | **`apt2sb`** | Nécessite les droits root et une session live MiniOS prise en charge. Les paquets sont installés dans un environnement de construction isolé, et non dans le root actif. | [Créer un module à partir de paquets](/preparing-and-customizing/Managing-Modules#create-a-module-from-packages); `man apt2sb` |
| Construire un module en exécutant un script d'installation | **`script2sb`** | Nécessite les droits root et une session live MiniOS prise en charge. Le script s’exécute sans interaction dans un environnement de construction isolé. | [Créer un module à partir d’un script](/preparing-and-customizing/Managing-Modules#create-a-module-from-a-script); `man script2sb` |
| Construire un module de façon interactive dans un environnement préparé | **`chroot2sb`** | Nécessite les droits root et une session live MiniOS prise en charge. À utiliser si l'installation nécessite des invites ou des modifications manuelles. | [Créer un module de façon interactive](/preparing-and-customizing/Managing-Modules#create-a-module-interactively); `man chroot2sb` |
| Convertir un arbre de répertoires en un `.sb` module | **`dir2sb`**, **`sb2dir`** | La conversion classique ne nécessite pas les droits root et peut être utilisée hors session live si les outils et fichiers requis sont disponibles. | [Créer](/preparing-and-customizing/Managing-Modules#create-a-module-from-a-directory) ou [extraire](/preparing-and-customizing/Managing-Modules#inspect-and-extract-modules) un module ; `man dir2sb`, `man sb2dir` |
| Capturer les modifications éligibles de la couche session accessible en écriture dans un module | **`savechanges`** | Nécessite les droits root et une session live MiniOS en cours avec un backend de couche en écriture pris en charge. | [Capturer les modifications de la session en cours](/preparing-and-customizing/Managing-Modules#capture-current-session-changes); `man savechanges` |
