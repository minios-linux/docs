...
| `perchsize` | Bei jedem Start | Container-Größe für `dynfilefs`, `dynblk`, `raw` und `luks`; gilt nicht für `native` oder `squashfs`. Eine reine Zahl oder `M`/`MB`-Wert wird in MiB zugewiesen; `G`/`GB` und `T`/`TB` werden zu 1000 bzw. 1.000.000 MiB umgerechnet. Dynblk verwendet standardmäßig ein schlankes 16 GiB-Limit und hat eine virtuelle Obergrenze von 512 GiB; der physische Speicher wächst bei Bedarf. Andere Container-Anfragen sind auf 1.000.000 MiB und den verfügbaren Speicher nach `perchreserve` begrenzt. MiniOS-Sitzungsmanager begrenzt Raw- und LUKS-Dateien auf 4000 MiB bei FAT32; für initrd LUKS gilt diese Grenze ebenfalls, aber eine zu große initrd Raw-Anfrage kann fehlschlagen, anstatt reduziert zu werden. Neue Raw- und LUKS-Container haben standardmäßig 4000 MiB. Initramfs-erstellte DynFileFS verwenden standardmäßig die verfügbare Kapazität, abgerundet auf 1000 MiB; MiniOS-Sitzungsmanager setzt hier 4000 MiB als Standard. | `perchsize=4000`<br>`perchsize=32GB`<br>`perchsize=512GB` |
...
| `perchmode` | Bei jedem Start | Persistenz-Speichermodus.<br>`native` (Standard): Ein Verzeichnis auf einem beschreibbaren POSIX-Dateisystem.<br>`dynfilefs`: Das FUSE-basierte, erweiterbare format-400-Containerformat, auch auf FAT32, NTFS oder exFAT.<br>`dynblk`: Ein separates format-1 Kernel-Blockgerät, das von schlanken `volumeNNN.db`-Dateien unterstützt wird; die tatsächliche `/dev/dynblkN`Anzahl wird dynamisch zugewiesen und mehrere Volumes können gleichzeitig existieren.<br>`raw`: Ein ext4-Image mit fester Größe.<br>`luks`: Ein LUKS2-verschlüsselter ext4-Container; Erstellung und Entsperren erfolgen über die Konsole und erfordern Crypt-Unterstützung im Initramfs.<br>`squashfs`: Ein vorhandener, komprimierter Snapshot wird für die Sitzung entpackt. MiniOS-Sitzungsmanager kann SquashFS-Snapshots des laufenden Systems erstellen und speichern; das Initramfs kann Snapshots wiederherstellen, aber nicht erstellen. | `perchmode=native`<br>`perchmode=dynfilefs`<br>`perchmode=dynblk`<br>`perchmode=raw`<br>`perchmode=luks`<br>`perchmode=squashfs` |
