...
- **Права**: требуется root-доступ (`sudo`)
- **Память**: достаточно ОЗУ для временных файлов
- **Диск**: свободное место для выходного ISO (обычно 1–4 ГБ)

### Требование к загрузочным файлам

Для работы **sb2iso** необходимы загрузочные файлы. Если система загружена в ОЗУ, используйте:
