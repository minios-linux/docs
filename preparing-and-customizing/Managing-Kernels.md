...
This page covers the modular MiniOS live system. MiniOS Kernel Manager and `minios-kernel` exist for that live architecture and its coordinated kernel module, `vmlinuz`, and initramfs set. After native conversion, the system uses a conventional Debian kernel workflow; the MiniOS live kernel tools are removed because the modular kernel model no longer applies, so use Debian kernel packages, initramfs tools, and the installed bootloader instead. See [About MiniOS](/getting-started/About-MiniOS) and [Boot modes](/using-minios/Boot-Modes). For the live initrd's exact coordinated kernel behavior, see [Initrd module loading](/reference/boot-process/Module-Loading).
...
- **What to do:** Boot a known-good MiniOS system, preserve important data, and restore a complete known-good kernel set only if one is available. Otherwise, reinstall the affected MiniOS installation. Do not try to repair the system by mixing individual kernel, initramfs, or `01-kernel-*.sb` files. See [Troubleshooting](/maintenance-and-recovery/Troubleshooting).
...
#### **Recovering after a failed kernel change**
Do not copy an individual kernel image, initramfs, or `01-kernel-*.sb` module from another image. A bootable MiniOS kernel requires the coordinated set. If a complete known-good set is not already available through the kernel-management workflow, reinstall the affected MiniOS installation rather than assembling boot components by hand. Preserve important data first; see [Troubleshooting](/maintenance-and-recovery/Troubleshooting).
