...
MiniOS releases are published on the official [GitHub Releases page](https://github.com/minios-linux/minios-live/releases) and [SourceForge](https://sourceforge.net/projects/minios-linux/). Each ISO has a matching file whose name ends in `.iso.sha256`.
...
Download the ISO and its matching `.sha256` file from the same release on GitHub or SourceForge. Keep both files in the same directory. Their base names must match, for example:
