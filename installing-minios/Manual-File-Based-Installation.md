...
This MiniOS installation method copies the system files to a normal filesystem and installs the bootloader there instead of writing the ISO block-for-block.
The remaining space on the filesystem stays available for ordinary files, so the device can continue to be used as a normal USB drive as well as a MiniOS boot device.

This method is especially useful when you want direct access to the MiniOS files, normal data storage on the same partition, or a writable layout for persistent sessions. It uses the SYSLINUX bootloader and is intended for Windows and Linux.
...
`bootinst` writes boot code to the disk derived from the installer's location. Read [Troubleshooting](/maintenance-and-recovery/Troubleshooting) before changing boot code, and do not run the installer until its device and mount point have been verified.
...
The chosen filesystem affects which persistence backends can work, but it does not enable persistence or guarantee that a session will be created. Persistence is enabled only when a boot entry or kernel command line requests it, and activation still requires suitable writable storage. See [Boot modes](/using-minios/Boot-Modes) and [Initrd persistence](/reference/boot-process/Persistence-Internals) before relying on saved changes.
