...
| `perchsize` | Every boot | Container size for `dynfilefs`, `dynblk`, `raw`, and `luks`; it does not apply to `native` or `squashfs`. A bare number or `M`/`MB` value is allocated in MiB; `G`/`GB` and `T`/`TB` are converted to 1000 and 1,000,000 MiB. Dynblk uses a thin 16 GiB default and has a 512 GiB virtual-capacity ceiling; its physical backing grows on demand. Other container requests are capped at 1,000,000 MiB and by available space after `perchreserve`. MiniOS Session Manager caps raw and LUKS files at 4000 MiB on FAT32; initrd LUKS applies that cap, but an oversized initrd raw request can fail instead of being reduced. New raw and LUKS containers default to 4000 MiB. Initramfs-created DynFileFS defaults to available capacity rounded down to 1000 MiB; MiniOS Session Manager defaults it to 4000 MiB. | `perchsize=4000`<br>`perchsize=32GB`<br>`perchsize=512GB` |
...
| `perchmode` | Every boot | Persistence storage mode.<br>`native` (default): a directory on a writable POSIX filesystem.<br>`dynfilefs`: the FUSE-based expandable format-400 container, including on FAT32, NTFS, or exFAT.<br>`dynblk`: a separate format-1 kernel block device backed by thin `volumeNNN.db` files; the actual `/dev/dynblkN` number is allocated dynamically and several volumes can coexist.<br>`raw`: a fixed-size ext4 image.<br>`luks`: a LUKS2-encrypted ext4 container; creation and unlock prompt on the console and require crypt support in the initramfs.<br>`squashfs`: an existing compressed snapshot unpacked for the session. MiniOS Session Manager can create and save SquashFS snapshots from the running system; the initramfs can resume but cannot create them. | `perchmode=native`<br>`perchmode=dynfilefs`<br>`perchmode=dynblk`<br>`perchmode=raw`<br>`perchmode=luks`<br>`perchmode=squashfs` |
