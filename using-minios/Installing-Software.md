...
MiniOS Store provides a catalog of application recipes at [store.minios.dev](https://store.minios.dev). In the MiniOS live environment, those recipes can be installed directly into the running system or used to build one or more SquashFS (`.sb`) modules.

This page describes the MiniOS live software workflow. A native installation keeps the selected desktop and ordinary applications, but removes the MiniOS-specific software intended for live operation. Use the normal Debian package-management workflow on that installed system instead.
...
4. On a live MiniOS session, select `Module` or `System`.
...
System mode uses APT or a recipe script directly against the running root filesystem. Changes take effect in the current live system rather than producing a module. Whether those changes survive a reboot depends on the session's persistence configuration.
...
- Access MiniOS Store only through its official HTTPS website and inspect recipes before installation.
